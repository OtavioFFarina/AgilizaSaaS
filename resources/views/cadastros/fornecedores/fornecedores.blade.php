...
@section('content')
    ...
    <div class="container-fluid px-4 px-md-5 pb-5" style="max-width: 1200px;">
        ...
        <div class="d-flex justify-content-between align-items-end mb-4">
            <h1 class="page-title"><i class='bx bxs-truck text-primary me-2'></i> Gerenciar Fornecedores</h1>
        </div>

        <div class="row g-4 mb-4">
            <div class="col-12 border-0">
                <div class="form-card">
                    <h5 class="section-title">Novo Fornecedor</h5>
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-12 mt-4 pt-2">
                                <button type="submit"
                                    ...
                                    <i class='bx bx-check-circle fs-5'></i> Cadastrar Fornecedor
                                </button>
                            </div>
                        </div>
                    </form>
        ...
        <div class="table-container p-0">
            <div class="d-flex justify-content-between align-items-center p-4 pb-3 border-bottom border-light">
                <h5 class="m-0 font-weight-600">Fornecedores Cadastrados</h5>
            </div>
                ...
                <table class="table-modern mb-0">
                    ...
                    <tbody>
                        @forelse ($fornecedores as $fornecedor)
                            <tr class="{{ !$fornecedor->ativo ? 'opacity-50' : '' }}"
                                style="background-color: {{ !$fornecedor->ativo ? 'var(--bg-body)' : 'transparent' }}">
                                <td class="fw-bold text-dark">{{ $fornecedor->nome_fornecedor }}</td>
                                <td class="text-muted">{{ $fornecedor->cnpj ?? '-' }}</td>
                                <td>
                                ...
                                <td class="text-end pe-4">
                                    <div class="d-flex justify-content-end gap-2">
                                        ...
                                        <form action="{{ route('fornecedores.toggle', $fornecedor->id) }}" method="POST"
                                            class="d-inline m-0 p-0">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
                                                class="btn btn-sm btn-light py-1 px-2 border-0 {{ $fornecedor->ativo ? 'text-success' : 'text-muted' }}"
                                        ...
                                        <button type="button" onclick="confirmarExclusao({{ $fornecedor->id }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center p-5 text-muted">
                                    ...
                                    <p class="m-0">Nenhum fornecedor cadastrado.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <script>
        function confirmarExclusao(id) {
            Swal.fire({
                title: 'Tem certeza?',
                text: "O fornecedor será removido do sistema.",
                icon: 'warning',
